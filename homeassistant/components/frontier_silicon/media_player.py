...
SERVICE_SELECT_PRESET_SCHEMA = cv.make_entity_service_schema(
...
async def async_setup_entry(
    ...
    async def async_handle_select_preset_service_call(
        ...
        for entity in entities:
            assert isinstance(entity, AFSAPIDevice)
            await entity.async_select_preset(preset)

    hass.services.async_register(
        DOMAIN,
        SERVICE_SELECT_PRESET,
        async_handle_select_preset_service_call,
        SERVICE_SELECT_PRESET_SCHEMA,
    )

    webfsapi_url = config_entry.data[CONF_WEBFSAPI_URL]
    pin = config_entry.data[CONF_PIN]
    use_session = config_entry.data[CONF_USE_SESSION]

    try:
        afsapi = AFSAPI(webfsapi_url, pin, use_session)
        async_add_entities([AFSAPIDevice(config_entry, afsapi)], True)
    except requests.exceptions.RequestException:
        _LOGGER.error(f"Could not add the FSAPI device {webfsapi_url}")


class AFSAPIDevice(MediaPlayerEntity):
    """Representation of a Frontier Silicon device on the network."""

    def __init__(self, config_entry: ConfigEntry, afsapi: AFSAPI):
    ...
    async def async_update(self):
        ...
        if self._attr_state != STATE_OFF:
            ...
            source = (await afsapi.get_mode()).label

            if source != self._attr_source:
                await self._async_update_presets()
    ...
    async def _async_update_presets(self):
        self.__presets_by_label = {
            preset.name: preset.name for preset in await self._afsapi.get_presets()
        }
        self._attr_preset_list = list(self.__presets_by_label.keys())
    ...
    @property
    def preset_list(self) -> list[str] | None:
        """Return the available presets."""
        return self._attr_preset_list

    async def async_select_preset(self, preset):
        """Select preset."""
        await self._afsapi.set_power(True)
        await self._afsapi.select_preset(preset)
